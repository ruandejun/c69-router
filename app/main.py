...
from app.network_setup import (
    ...
    detect_lan_interface, is_lan_interface_valid, smart_detect_lan,
)
...
def setup_windows_startup():
    ...
    try:
        ...
        logger.warning(f"[Startup] Loi khi dang ky khoi dong cung Windows: {e}")


# ─── SETUP_WINDOWS_BAT content (nhung vao exe de tu tao neu chua co) ──────────
_SETUP_BAT_CONTENT = r"""@echo off
...
# ─── SETUP_WINDOWS_PS1 content (nhung vao exe de tu tao neu chua co) ──────────
# Noi dung day du cua setup_windows.ps1 — duoc nhung vao binary de tu sinh ra
# khi can, nguoi dung khong can phai copy file rieng len may moi.
_SETUP_PS1_CONTENT = r"""# ============================================================
...
def _run_first_time_setup():
    ...
    Logic:
    1. Kiem tra file marker 'data/.setup_done' — neu ton tai: da setup roi, bo qua.
    2. Neu chua co marker:
       a. Neu file setup_windows.bat / setup_windows.ps1 chua co -> tu tao tu noi dung nhung san.
       b. Chay setup_windows.bat (elevated, non-blocking — bat len roi de no tu chay).
       c. Tao marker data/.setup_done.
    3. Neu needReboot (Hyper-V moi duoc kich hoat): chi warn, khong force reboot.
       c69-router se tiep tuc chay nhung NAT co the chua hoat dong den khi reboot.
    """
    if platform.system() != "Windows":
        return

    # Chi chay khi dong goi exe (frozen) hoac co the chay khi dev mode de test
    data_dir = os.path.join(PROJECT_DIR, "data")
    os.makedirs(data_dir, exist_ok=True)
    marker_path = os.path.join(data_dir, ".setup_done")

    if os.path.exists(marker_path):
        logger.debug("[Setup] First-run setup da hoan thanh truoc do, bo qua.")
        return

    logger.info("[Setup] *** FIRST RUN DETECTED — bat dau thiet lap Windows tu dong ***")

    # Xac dinh thu muc chua exe (hoac thu muc project khi dev)
    if getattr(sys, "frozen", False):
        base_dir = os.path.dirname(sys.executable)
    else:
        base_dir = PROJECT_DIR

    bat_path = os.path.join(base_dir, "setup_windows.bat")
    ps1_path = os.path.join(base_dir, "setup_windows.ps1")

    # Tao file setup neu chua co (may moi cai c69-router.exe don gian)
    try:
        if not os.path.exists(bat_path):
            logger.info(f"[Setup] setup_windows.bat chua co -> tu tao: {bat_path}")
            with open(bat_path, "w", encoding="utf-8", newline="\r\n") as f:
                f.write(_SETUP_BAT_CONTENT)

        if not os.path.exists(ps1_path):
            logger.info(f"[Setup] setup_windows.ps1 chua co -> tu tao: {ps1_path}")
            with open(ps1_path, "w", encoding="utf-8", newline="\r\n") as f:
                f.write(_SETUP_PS1_CONTENT)
    except Exception as e:
        logger.error(f"[Setup] Khong tao duoc setup files: {e}")
        return

    # Chay setup_windows.bat — elevated (Start-Process RunAs) de co quyen Admin
    # Chay NON-BLOCKING: c69-router tiep tuc khoi dong song song.
    # setup_windows.ps1 tu hien thi ket qua trong cua so rieng.
    try:
        logger.info(f"[Setup] Dang chay: {bat_path}")
        subprocess.Popen(
            ["powershell", "-NoProfile", "-Command",
             f"Start-Process -FilePath '{bat_path}' -Verb RunAs -WindowStyle Normal"],
            creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, "CREATE_NO_WINDOW") else 0
        )
        logger.info("[Setup] setup_windows.bat da duoc khoi dong (cua so rieng).")
    except Exception as e:
        logger.warning(f"[Setup] Khong the chay setup_windows.bat tu dong: {e}. "
                       f"Vui long chay thu cong: {bat_path}")

    # Tao marker — danh dau da chay setup lan dau
    # (ke ca setup co fail 1 so buoc, c69-router van tiep tuc — user xem log cua so setup)
    try:
        with open(marker_path, "w") as f:
            import datetime
            f.write(f"setup_done={datetime.datetime.now().isoformat()}\n")
        logger.info(f"[Setup] Marker tao tai: {marker_path}")
    except Exception as e:
        logger.warning(f"[Setup] Khong tao duoc marker: {e}")
...
async def lifespan(app: FastAPI):
    ...
    # 2.5 Setup auto-startup (dang ky Task Scheduler)
    setup_windows_startup()

    # 2.6 FIRST RUN: tu dong chay setup_windows.bat neu chua chay lan nao
